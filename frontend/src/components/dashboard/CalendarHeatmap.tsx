...
export default function CalendarHeatmap({ points }: { points: HeatmapPoint[] }) {
  ...
  const cells: ({ date: string; amount: number } | null)[][] = Array.from({ length: 7 }, () => []);
  ...
  for (let d = new Date(first); d <= last; d.setDate(d.getDate() + 1)) {
    ...
    cells[dow].push(point ? { date: iso, amount: point.amount } : { date: iso, amount: 0 });
  }
  // Pad trailing
  ...
  return (
    <div className="relative">
      ...
      {hover && (
        <div className="absolute top-0 right-0 -mt-8 rounded-md border border-border bg-background px-2 py-1 text-[10px] font-mono shadow-sm">
